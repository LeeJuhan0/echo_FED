...
def evaluate_target(dat: pd.DataFrame, target: str, sentiment_col: str,
    ...
    Rolling OOS 평가:
      ...
      - ARIMAX (global order, exog=[sentiment, prev])
      - ARIMA  (global order, no exog)
    ARIMAX/ARIMA 차수는 전체 표본으로 한 번 선택 후 매 시점 재적합.
    """
    ...
    for te in range(window_size, n):
        ...
        if len(y_tr) < 2:
            ...
            # ARX(1): y_dep = c + φ y_lag + β * sent_curr (sent_curr aligned with y_dep)
            sent_curr = sent_tr[1:]
            ...
            # Forecast uses last y_{t-1} and sentiment_t
            pred_arx_ols = c_ax + phi_ax * y_prev_last + beta_ax * sent_ts

        # ---------------- ARIMAX (global order) ----------------
        exog_tr = np.column_stack([sent_tr, prev_tr])
        exog_ts = np.array([[sent_ts, prev_ts]])
        try:
            model_arimax = SARIMAX(endog=y_tr, exog=exog_tr, order=order_arimax, trend="c",
                                   enforce_stationarity=False, enforce_invertibility=False)
            res_arimax = model_arimax.fit(disp=False)
            fcast_arimax = res_arimax.get_forecast(steps=1, exog=exog_ts).predicted_mean
            pred_arimax = _first_pred(fcast_arimax)
        except Exception:
            pred_arimax = prev_ts  # 안전 fallback
        ...
        try:
            ...
            fcast_arima = res_arima.get_forecast(steps=1).predicted_mean
            pred_arima = _first_pred(fcast_arima)
        except Exception:
            pred_arima = prev_ts
