...
def _global_best_orders(dat: pd.DataFrame, y_col: str, sentiment_col: str, prev_col: str,
    ...
    if "month" not in dat.columns:
        raise KeyError("[global_orders] 'month' column is required.")
...
def evaluate_target(dat: pd.DataFrame, target: str, sentiment_col: str,
                    ...
                    base_window=30, small_window=15,
    ...
    # --- 글로벌 ARIMAX / ARIMA order 선택 ---
    order_arimax, order_arima = _global_best_orders(df_all, y_col, sentiment_col, prev_col,
                                                    p_list=p_list, d_list=d_list, q_list=q_list)
    ...
    for te in range(window_size, n):
        ...
        # ---------------- AutoReg ARX ----------------
        if arx_use_prev:
            exog_tr_arx = np.column_stack([sent_tr, prev_tr])
            exog_ts_arx = np.array([[sent_ts, prev_ts]])
        else:
            exog_tr_arx = sent_tr.reshape(-1,1)
        ...
        try:
            ...
            arx_pred = y_tr[-1]
            print("arx예외 발생")
        # ---------------- ARIMAX (global order, sentiment+prev) ----------------
        exog_tr_glob = np.column_stack([sent_tr, prev_tr])
        exog_ts_glob = np.array([[sent_ts, prev_ts]])
        try:
            model_arimax = SARIMAX(endog=y_tr, exog=exog_tr_glob, order=order_arimax, trend="c",
                                   enforce_stationarity=False, enforce_invertibility=False).fit(disp=False)
            pred_arimax = _first_pred(model_arimax.get_forecast(steps=1, exog=exog_ts_glob).predicted_mean)
        except Exception:
            pred_arimax = prev_ts

        # ---------------- ARIMA (global order, no exog) ----------------
        try:
            model_arima = SARIMAX(endog=y_tr, exog=None, order=order_arima, trend="c",
                                  enforce_stationarity=False, enforce_invertibility=False).fit(disp=False)
            pred_arima = _first_pred(model_arima.get_forecast(steps=1).predicted_mean)
        except Exception:
            pred_arima = prev_ts
        ...
        rows.append({
            ...
            "y": y_ts,
            "arx_mle": arx_pred,      # 이름 유지(기존 인터페이스와 호환), 실제는 AutoReg ARX
            "ar_mle": ar_pred,        # 이름 유지, 실제는 AutoReg AR
            "arima_w": pred_arimax,
            "arima_wo": pred_arima,
            "ar_lags": ar_lags,
            "arx_lags": arx_lags
    ...
    labels = [
        ("ARX AutoReg (lags={}, sentiment{})"
         .format(arx_lags, "+prev" if arx_use_prev else ""), "arx_mle"),
        ("AR AutoReg (lags={})".format(ar_lags), "ar_mle"),
        ("ARIMAX (global order, sentiment+prev)", "arima_w"),
        ("ARIMA  (global order, no exog)", "arima_wo"),
    ]
    ...
    return {
        ...
        "orders": {
            "AR_lags": ar_lags,
            "ARX_lags": arx_lags,
            "ARIMAX_global": order_arimax,
            "ARIMA_global": order_arima
        },
